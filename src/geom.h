...
typedef struct{
    ...
    double* dat;     //pointer to the elements of the matrix/vector
}object;

extern int precision; //global variable for data printing precision
...
object* lin_comb_obj(object a, object b, const double a_coef, const double b_coef, const char* new_name); //creates a new matrix= a_coef*a + b_coef*b
double det(object m); //determinant of m
object* scalar_mul(double x, object M, const char* new_name); //multiplies M and a real number x
...
double dot_prod(object u, object v); //dot product of vectors u and v
double norm(object v); //norm of v
...
double tr(object M); //trace of m 
object* qr_diag_obj(object M,int max_iter, const char* new_name, int *error_code,double* r_eig, int* n_rl, double* RE, double* IM, int* n_compl); //computes eigenvalues of matrix M with QR algorithm. Look at the code for details.